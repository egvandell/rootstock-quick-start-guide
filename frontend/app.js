document.addEventListener('DOMContentLoaded', function () {
    ...
    let contractABI = [];
    let networks = {};
    const contractAddress = `0xbf8c06958eb5a800aa0d22d40e342a1db22b6263`;
   ...
   */
    async function fetchExternalFiles() {
      ...
      let response = await fetch('MyConsensus2024Token.json');
    ...
    fetchExternalFiles().then(() => {
      ...
      getBalanceButton.addEventListener('click', async function () {
        ...
        if (myTokenContract) {
          ...
          walletBalanceDiv.innerHTML = `MyToken Balance: ${balance}`;
        }
      });
    });
  });